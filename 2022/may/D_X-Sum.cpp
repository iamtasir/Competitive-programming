...
#define mod 1000000007
ll ar[MAX][MAX];
ll sum(ll n, ll m, ll posx, ll posy)
{

    ll ans = 0;
    for (ll i = posx, j = posy; i <= n && j <= m; i++, j++)
    {
        ans += ar[i][j];
    }

    debug(ans);
    for (ll i = posx, j = posy; i >= 1 && j >= 1; i--, j--)
    {

        ans += ar[i][j];
    }
    debug(ans);
    for (ll i = posx, j = posy; i <= n && j >= 1; i++, j--)
    {

        ans += ar[i][j];
    }
    debug(ans);
    for (ll i = posx, j = posy; i >= 1 && j <= m; i--, j++)
    {
        debug(ar[i][j]);
        debug(i);
        debug(j);
