...
node Merge(node a, node b)
{
    ...
    if (a.lmx.sc == a.rmx.sc && a.rmx.sc == b.lmx.sc)
    {
        c.lmx = {a.lmx.first + b.lmx.first, a.lmx.sc};
    }
    else
        c.lmx = a.lmx;

    if (b.lmx.sc == b.rmx.sc && b.lmx.sc == a.rmx.sc)
    {
        c.rmx = {a.rmx.first + b.lmx.first, b.rmx.second};
    }
    else
    {
        c.rmx = b.rmx;
    ...
    {
        c.mx = max({c.mx, {a.rmx.ff + b.lmx.first, b.lmx.sc}, c.lmx, c.lmx});
    }
...
void build(ll node, ll st, ll ed)
{
    if (st == ed)
    {
        tree[node].lmx = {1, ar[st]};
        tree[node].rmx = {1, ar[st]};
        tree[node].mx = {1, ar[st]};
...
ll inf = -1e17;
node query(ll nd, ll st, ll ed, ll l, ll r)
{
    if (st > r || ed < l)
    {
        node temp;
    ...
    return Merge(query(nd * 2, st, mid, l, r), query(nd * 2 + 1, mid + 1, ed, l, r));
}
...
int main()
{
    ...
    while (t--)
    {
        ...
        while (q--)
        {
            ll l, r;
            cin >> l >> r;
            node value = query(1, 0, n - 1, l - 1, r - 1);
            cout << value.mx.ff << nn;
        }
    }
