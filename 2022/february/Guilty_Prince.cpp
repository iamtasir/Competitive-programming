...
ll vis[mx][mx];
ll starti, startj;
void depthfirstsearch(ll u, ll v)
{
    cnt++;
    vis[u][v] = 1;
    for (ll i = 0; i < 4; i++)
    {
        if (!vis[u + dx[i]][v + dy[i]] && u + dx[i] >= 0 && u + dx[i] < n && v + dy[i] >= 0 && v + dy[i] < m && ch[u + dx[i]][v + dy[i]] != '#')
            depthfirstsearch(u + dx[i], v + dy[i]);
    }
}
int main()
{
    ...
    while (t--)
    {
        ...
        cnt = 0;
        depthfirstsearch(starti, startj);
        cout << "Case " << cs++ << ": " << cnt << nn;
        for (ll i = 0; i < n; i++)
            for (ll j = 0; j < m; j++)
                vis[i][j] = 0;
