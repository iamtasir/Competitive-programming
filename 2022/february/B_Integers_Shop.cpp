...
int main()
{
    ...
    while (t--)
    {
        ll n;
        cin >> n;
        ll left = INT_MAX, right = INT_MIN, costleft = 0, costright = 0;
        ll c = 0;
        map<pll, ll> mp;
        for (ll i = 1; i <= n; i++)
        {
            ...
            if (mp[{left, right}] != 0)
            {
                cout << min(mp[{left, right}], costleft + costright) << nn;
            }
            else
            {
                cout << costright + costleft << nn;
            }
        }
    }
