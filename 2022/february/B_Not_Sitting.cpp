...
int main()
{
    ...
    while (t--)
    {
        ...
        for (ll i = 1; i <= n; i++)
        {
            for (ll j = 1; j <= m; j++)
            {
                ll temp = abs(1 - i) + abs(1 - j);  //(1,1);
                ll temp2 = abs(1 - i) + abs(m - j); //(1,m);
                ll temp3 = abs(n - i) + abs(1 - j); //(n,1);
                ll temp4 = abs(n - i) + abs(m - j); //(n,m);
                ll ans = max(temp, max(temp2, max(temp3, temp4)));
                v.pb(ans);
            }
        }
        sort(all(v));
