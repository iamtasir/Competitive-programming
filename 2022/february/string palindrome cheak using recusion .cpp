...
#define MAX 1000005
#define mod 1000000007
bool palindromecheak(string &s, ll n, ll index)
{
    if (index > n / 2)
        return 1;
    if (s[index] == s[n - index - 1])
        palindromecheak(s, n, index + 1);
    else
        return 0;
}
int main()
{
    ...
    while (t--)
    {
        ...
        cin >> s;
        ll index = 0;
        if (palindromecheak(s, sz(s), index))
            cout << "string is palindrome " << nn;
        else
            cout << "string is not palindrome" << nn;
    }
