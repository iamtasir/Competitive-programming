...
ll ar[10]={1,1,5,-1,-1,2,-1,-1,8,-1};
 ll h,m;
 bool cheak(ll x,ll y)
 {
     if(ar[x/10]==-1||ar[x%10]==-1||ar[y/10]==-1||ar[y%10]==-1)
        return false ;
     ll x1=ar[y%10]*10+ar[y/10],
     y1=ar[x%10]*10+ar[x/10];
    // cout<<x1<<ss<<y1<<nn;
     if(x1<h&&y1<m) return true;
     else return false;
