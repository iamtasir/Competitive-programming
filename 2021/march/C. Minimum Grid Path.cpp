...
int main()
{
 //O_O ;
   ...
   for(ll i=0;i<n;i++)
    cin>>ar[i];
   multiset<ll>m1,m2;
   m1.insert(ar[0]),m2.insert(ar[1]);
   ll  ans1=ar[0]*n,ans2=ar[1]*n;
   ll ans=ans1+ans2;
  ll cost1=ar[0],cost2=ar[1];
  for(ll i=2;i<n;i++)
  {
      if(i&1)
      {
        ...
        m1.insert(ar[i]),cost1+=ar[i];

      ll index1=*m1.begin(),index2=*m2.begin();
      ll rem1=n-m1.size(),rem2=n-m2.size();
      ll temp1=cost1+rem1*index1,
      temp2=cost2+rem2*index2;
