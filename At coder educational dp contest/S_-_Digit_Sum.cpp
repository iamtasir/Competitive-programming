...
/*
 Algorithm Used:digital dp theory
 here we have to cheack for every index individual
 0-9 then we have to answer for very index of n and answer the sum of the index
 here state size,divisorvalue,bool cheak
...
ll n;
ll dp[200005][105][2];
ll go(ll len, ll reminder, ll state)
{
    if (len == n + 1)
        return reminder == 0;

    if (dp[len][reminder][state] != -1)
        return dp[len][reminder][state];
    ...
    {
        if (state && i == range)
            ans = ((ans + go(len + 1, (reminder + i) % k, 1)) % mod);
        else
            ans = ((ans + go(len + 1, (reminder + i) % k, 0)) % mod);
    }

    ans %= mod;
    dp[len][reminder][state] = ans;
    return ans;
}
int main()
{
    ...
    // for (ll i = 1; i <= n; i++)
    //     cout << ar[i] << ss;
    cout << ((go(1, 0, 1) - 1) + mod) % mod << nn; //////bcz all zero 000000000000 value is a number
    // for (ll i = 1; i <= n; i++)
    // {
    //     for (ll j = 0; j <= 9; j++)
    //     {
