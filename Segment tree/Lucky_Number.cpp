...
/*
 Algorithm Used:  trix walking in the tree
  first set all the value 1 for every index,then just delete all the element for every index value,then
  then just using sieve method to delete all the element form the update array
...
ll query(ll start, ll end, ll node, ll idx)
{
    ...
    if (tr[2 * node] >= idx)
    {
        return query(start, mid, 2 * node, idx);
    }
    else
    {
        return query(mid + 1, end, 2 * node + 1, idx - tr[2 * node]);
    }
}
...
int main()
{
    ...
    while (index <= 1e5)
    {
        ...
        ll index1 = query(1, MAX, 1, index);
        // debug(index1);

        ll value = ar[index1];
        // debug(value);
        vll v;
        for (ll j1 = value; j1 <= MAX; j1 += value)
        {
            if (j1 > cnt)
            {
                break;
            }
            ll tempindex = query(1, MAX, 1, j1);
            v.pb(tempindex);
            cnt--;
        }
        // debug(v);
